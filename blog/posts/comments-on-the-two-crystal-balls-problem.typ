...
#set document(title: "Comment on the Two Crystal Balls problem")
#title()

The Two Crystal Balls problem, as mentioned in e.g. #link("https://frontendmasters.com/courses/algorithms/two-crystal-balls-problem/")[this course], is the following problem:
...
Firstly, it is possible to show that this is the optimum. From #link("http://www.tanyakhovanova.com/Puzzles/solballs.html")[Tanya Khovanova's page]:

Suppose we have a strategy which always terminates in at most $N$ throws. Then the floor is defined by two numbers: the number of throws before first ball breaks, and similarly for the the second ball. The sum of these two numbers should be less than or equal to $N$. Hence, the number of floors we can distinguish is the number of different pairs of such numbers, which is equal to $1/2 N(N+1)$.
...
We use each ball to make $N$ jumps, so the total number of jumps is $K N=O(n^(1-1/K))$.
...
It is amusing that in the limit of infinite balls $K->oo$, we formally obtain $O(1)$ which is off by a log (binary search). This "$n^0=log n$" appears in many places in mathematics. The simplest instance of this that I know of is the fact that the integral of $x^k$ is $x^(k+1)$ (times a constant) unless $k=-1$, in which case it is $log x$. Some other occurences can be found in #link("https://math.stackexchange.com/q/2632349/80734")[this Math.SE post].
...
```
As noted in #link("https://github.com/ThePrimeagen/kata-machine/pull/37")[this PR], this code doesn't work in some edge cases. The fix proposed is to replace the last return with
```typescript
